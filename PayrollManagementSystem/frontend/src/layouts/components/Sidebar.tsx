...
interface MenuItem {
  ...
  children?: { path: string; label: string; allowedRoles: string[]; icon?: React.ReactNode }[];
}
...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25M9 16.5h.01M9 20.25H15m-6-3H15m-3-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
          </svg>
        ),
      }
    ]
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: "/performance/kpi/duyet-kpi",
        label: "Phê Duyệt KPI",
        allowedRoles: ["Admin", "HR", "Employee"],
        requireManager: true,
        icon: (
          ...
          <ul className="sidebar-nav-list">
            {filteredMenuItems.map((item) => {
              ...
              let visibleChildren = item.children?.filter(child => {
                ...
                if (child.requireManager && !user?.hasDirectReports) return false;
                // Ẩn menu Duyệt Đánh giá nếu không có quyền quản lý (giữ lại code cũ để an toàn)
                if (child.path === "/performance/duyet-danh-gia" && !user?.hasDirectReports) {
                  return false;
                }
                return true;
              }) || [];
