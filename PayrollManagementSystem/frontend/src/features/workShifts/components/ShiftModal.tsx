            ...
            <div className="ws-modal-content">
                ...
                <form className="ws-form" onSubmit={handleSubmit}>
                    <div className="ws-form-grid">
                        <div className="ws-form-group ws-col-span-2">
                            <label>Tên ca làm việc <span className="ws-text-danger">*</span></label>
                            <input type="text" value={tenCa} onChange={e => setTenCa(e.target.value)} placeholder="Vd: Ca Hành Chính" className="ws-input" />
                        </div>
                        
                        <div className="ws-form-group">
                            <label>Giờ bắt đầu <span className="ws-text-danger">*</span></label>
                            <input type="time" value={gioBatDau} onChange={e => setGioBatDau(e.target.value)} className="ws-input" />
                        </div>

                        <div className="ws-form-group">
                            <label>Giờ kết thúc <span className="ws-text-danger">*</span></label>
                            <input type="time" value={gioKetThuc} onChange={e => setGioKetThuc(e.target.value)} className="ws-input" />
                        </div>

                        <div className="ws-form-group">
                            <label>Hệ số lương <span className="ws-text-danger">*</span></label>
                            <input type="number" step="0.1" value={heSoLuong} onChange={e => setHeSoLuong(parseFloat(e.target.value))} className="ws-input" />
                        </div>

                        <div className="ws-form-group ws-checkbox-group">
                            <label className="ws-checkbox-label">
                                <input type="checkbox" checked={xuyenNgay} onChange={e => setXuyenNgay(e.target.checked)} />
                                <span className="ws-checkbox-custom"></span>
                    ...
                    <div className="ws-breaks-section">
                        <div className="ws-breaks-header">
                            <h3>Khung giờ nghỉ</h3>
                            <button type="button" className="ws-btn-outline" onClick={handleAddBreak}>
                                + Thêm giờ nghỉ
                            </button>
                        </div>

                        {khungGioNghis.length === 0 ? (
                            <p className="ws-empty-text">Chưa có khung giờ nghỉ nào được cấu hình.</p>
                        ) : (
                            <div className="ws-breaks-list">
                                {khungGioNghis.map((b, index) => (
                                    <div key={index} className="ws-break-item">
                                        <div className="ws-form-group">
                                            <input type="text" placeholder="Tên giờ nghỉ" value={b.tenKhoangNghi} onChange={e => handleBreakChange(index, "tenKhoangNghi", e.target.value)} className="ws-input" />
                                        </div>
                                        <div className="ws-form-group">
                                            <input type="time" value={b.gioBatDau} onChange={e => handleBreakChange(index, "gioBatDau", e.target.value)} className="ws-input" />
                                        </div>
                                        <div className="ws-form-group">
                                            <input type="time" value={b.gioKetThuc} onChange={e => handleBreakChange(index, "gioKetThuc", e.target.value)} className="ws-input" />
                                        </div>
                                        <div className="ws-form-group ws-checkbox-center">
                                            <label className="ws-checkbox-label" title="Có tính thời gian nghỉ này vào tổng thời gian làm việc không?">
                                                <input type="checkbox" checked={b.tinhVaoGioLam} onChange={e => handleBreakChange(index, "tinhVaoGioLam", e.target.checked)} />
                                                <span className="ws-checkbox-custom"></span>
                                                Tính giờ làm
                                            </label>
                                        </div>
                                        <button type="button" className="ws-btn-icon-danger" onClick={() => handleRemoveBreak(index)}>
                    ...
                    <div className="ws-modal-footer">
                        <button type="button" className="ws-btn-secondary" onClick={onClose} disabled={isLoading}>
                            Hủy bỏ
                        </button>
                        <button type="submit" className="ws-btn-primary" disabled={isLoading}>
